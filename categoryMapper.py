...
if shouldSaveFeatureMappings == 'y':
    ...
    for j in stringColumns:
        ...
        if shouldSaveFeatureMappings == 'y':
            ...
            feature_sorted = sorted(distinctValues)
            featureMap = list(map(lambda x: f"{x[1]}, {x[0]}", enumerate(feature_sorted)))
    ...
    if shouldContinue == 'y':
        directory_featureMapping = input("Please enter the path to the featureMappings folder\n")
